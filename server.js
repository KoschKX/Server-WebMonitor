  ...
  const interval = setInterval(async () => {
    ...
    if (ws.readyState === WebSocket.OPEN) {
      try {
        ...
        // Use the overall load if per-core is not available or inaccurate
        ...
        const metrics = {
          ...
          memory: {
            total: mem.total,
            used: mem.used,  // Actual used memory (excludes buffers/cache)
            free: mem.free,
            active: mem.active,
            available: mem.available,
            buffcache: mem.buffcache || 0,
            usedPercent: ((mem.total - mem.available) / mem.total * 100).toFixed(2),
